...
UCentralTextBlock::UCentralTextBlock(const FObjectInitializer& InObjectInitializer)
	: Super(InObjectInitializer) {

	ConstructorHelpers::FObjectFinderOptional<UDataTable> Obj(TEXT("/CentralUIPlugin/Data/DataTables/UI/Slate/TextBlock/DT_Slate_TextBlock.DT_Slate_TextBlock"));
	if (Obj.Succeeded()) {
...
void UCentralTextBlock::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent) {
...
void UCentralTextBlock::OnWidgetRebuilt() {
...
FDataTableRowHandle UCentralTextBlock::GetCentralStyle() const {
	return CentralStyle;
}

void UCentralTextBlock::SetCentralStyle(FDataTableRowHandle InStyle, bool bInLoadStyle) {
...
TArray<FString> UCentralTextBlock::GetCentralManagedPropertyNames() const {
	TArray<FString> Names;
	// As long as property names are equal on style and styled object, we can simply iterate.
	for (TFieldIterator<FProperty> It(FS_CentralTextBlockStyle::StaticStruct()); It; ++It) {
		if (It != nullptr) {
...
bool UCentralTextBlock::LoadCentralStyle() {
	const FS_CentralTextBlockStyle* DTPtr = CentralStyle.GetRow<FS_CentralTextBlockStyle>(TEXT("Loading Central Style"));
	if (DTPtr == nullptr) {
	...
	SetColorAndOpacity(DTPtr->ColorAndOpacity);
	SetFont(DTPtr->Font);
	SetStrikeBrush(DTPtr->StrikeBrush);
	SetShadowOffset(DTPtr->ShadowOffset);
	SetShadowColorAndOpacity(DTPtr->ShadowColorAndOpacity);
...
void UCentralTextBlock::ActOnCentralStyleDataTableChanged() {
	LoadCentralStyle();
}
