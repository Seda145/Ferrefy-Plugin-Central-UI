...
UCentralEditableText::UCentralEditableText(const FObjectInitializer& InObjectInitializer)
	: Super(InObjectInitializer) {

	ConstructorHelpers::FObjectFinderOptional<UDataTable> Obj(TEXT("/CentralUIPlugin/Data/DataTables/UI/Slate/EditableText/DT_Slate_EditableText.DT_Slate_EditableText"));
	if (Obj.Succeeded()) {
...
void UCentralEditableText::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent) {
...
void UCentralEditableText::OnWidgetRebuilt() {
...
FDataTableRowHandle UCentralEditableText::GetCentralStyle() const {
	return CentralStyle;
}

void UCentralEditableText::SetCentralStyle(FDataTableRowHandle InStyle, bool bInLoadStyle) {
...
TArray<FString> UCentralEditableText::GetCentralManagedPropertyNames() const {
	TArray<FString> Names;
	// As long as property names are equal on style and styled object, we can simply iterate.
	for (TFieldIterator<FProperty> It(FS_CentralEditableTextStyle::StaticStruct()); It; ++It) {
		if (It != nullptr) {
...
bool UCentralEditableText::LoadCentralStyle() {
	const FS_CentralEditableTextStyle* DTPtr = CentralStyle.GetRow<FS_CentralEditableTextStyle>(TEXT("Loading Central Style"));
	if (DTPtr == nullptr) {
...
void UCentralEditableText::ActOnCentralStyleDataTableChanged() {
	LoadCentralStyle();
}
