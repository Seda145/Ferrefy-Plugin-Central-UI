...
UCentralComboBoxString::UCentralComboBoxString(const FObjectInitializer& InObjectInitializer)
	: Super(InObjectInitializer) {

	ConstructorHelpers::FObjectFinderOptional<UDataTable> Obj(TEXT("/CentralUIPlugin/Data/DataTables/UI/Slate/ComboBoxString/DT_Slate_ComboBoxString.DT_Slate_ComboBoxString"));
	if (Obj.Succeeded()) {
...
void UCentralComboBoxString::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent) {
...
void UCentralComboBoxString::OnWidgetRebuilt() {
...
FDataTableRowHandle UCentralComboBoxString::GetCentralStyle() const {
	return CentralStyle;
}

void UCentralComboBoxString::SetCentralStyle(FDataTableRowHandle InStyle, bool bInLoadStyle) {
...
TArray<FString> UCentralComboBoxString::GetCentralManagedPropertyNames() const {
	TArray<FString> Names;
	// As long as property names are equal on style and styled object, we can simply iterate.
	for (TFieldIterator<FProperty> It(FS_CentralComboBoxStringStyle::StaticStruct()); It; ++It) {
		if (It != nullptr) {
...
bool UCentralComboBoxString::LoadCentralStyle() {
	const FS_CentralComboBoxStringStyle* DTPtr = CentralStyle.GetRow<FS_CentralComboBoxStringStyle>(TEXT("Loading Central Style"));
	if (DTPtr == nullptr) {
	...
	SetHasDownArrow(DTPtr->HasDownArrow);
...
void UCentralComboBoxString::ActOnCentralStyleDataTableChanged() {
	LoadCentralStyle();
}
