...
UCentralCircularThrobber::UCentralCircularThrobber(const FObjectInitializer& InObjectInitializer)
	: Super(InObjectInitializer) {

	ConstructorHelpers::FObjectFinderOptional<UDataTable> Obj(TEXT("/CentralUIPlugin/Data/DataTables/UI/Slate/CircularThrobber/DT_Slate_CircularThrobber.DT_Slate_CircularThrobber"));
	if (Obj.Succeeded()) {
...
void UCentralCircularThrobber::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent) {
...
void UCentralCircularThrobber::OnWidgetRebuilt() {
...
FDataTableRowHandle UCentralCircularThrobber::GetCentralStyle() const {
	return CentralStyle;
}

void UCentralCircularThrobber::SetCentralStyle(FDataTableRowHandle InStyle, bool bInLoadStyle) {
...
TArray<FString> UCentralCircularThrobber::GetCentralManagedPropertyNames() const {
	TArray<FString> Names;
	// As long as property names are equal on style and styled object, we can simply iterate.
	for (TFieldIterator<FProperty> It(FS_CentralCircularThrobberStyle::StaticStruct()); It; ++It) {
		if (It != nullptr) {
...
bool UCentralCircularThrobber::LoadCentralStyle() {
	const FS_CentralCircularThrobberStyle* DTPtr = CentralStyle.GetRow<FS_CentralCircularThrobberStyle>(TEXT("Loading Central Style"));
	if (DTPtr == nullptr) {
	...
	SetPeriod(DTPtr->Period);
	SetRadius(DTPtr->Radius);
...
void UCentralCircularThrobber::ActOnCentralStyleDataTableChanged() {
	LoadCentralStyle();
}
