...
UCentralSpinBox::UCentralSpinBox(const FObjectInitializer& InObjectInitializer)
	: Super(InObjectInitializer) {

	ConstructorHelpers::FObjectFinderOptional<UDataTable> Obj(TEXT("/CentralUIPlugin/Data/DataTables/UI/Slate/SpinBox/DT_Slate_SpinBox.DT_Slate_SpinBox"));
	if (Obj.Succeeded()) {
...
void UCentralSpinBox::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent) {
...
void UCentralSpinBox::OnWidgetRebuilt() {
...
FDataTableRowHandle UCentralSpinBox::GetCentralStyle() const {
	return CentralStyle;
}

void UCentralSpinBox::SetCentralStyle(FDataTableRowHandle InStyle, bool bInLoadStyle) {
...
TArray<FString> UCentralSpinBox::GetCentralManagedPropertyNames() const {
	TArray<FString> Names;
	// As long as property names are equal on style and styled object, we can simply iterate.
	for (TFieldIterator<FProperty> It(FS_CentralSpinBoxStyle::StaticStruct()); It; ++It) {
		if (It != nullptr) {
...
bool UCentralSpinBox::LoadCentralStyle() {
	const FS_CentralSpinBoxStyle* DTPtr = CentralStyle.GetRow<FS_CentralSpinBoxStyle>(TEXT("Loading Central Style"));
	if (DTPtr == nullptr) {
	...
	SetForegroundColor(DTPtr->ForegroundColor);
	SetFont(DTPtr->Font);
...
void UCentralSpinBox::ActOnCentralStyleDataTableChanged() {
	LoadCentralStyle();
}
