...
UCentralImage::UCentralImage(const FObjectInitializer& InObjectInitializer)
	: Super(InObjectInitializer) {

	ConstructorHelpers::FObjectFinderOptional<UDataTable> Obj(TEXT("/CentralUIPlugin/Data/DataTables/UI/Slate/Image/DT_Slate_Image.DT_Slate_Image"));
	if (Obj.Succeeded()) {
...
void UCentralImage::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent) {
...
void UCentralImage::OnWidgetRebuilt() {
...
FDataTableRowHandle UCentralImage::GetCentralStyle() const {
	return CentralStyle;
}

void UCentralImage::SetCentralStyle(FDataTableRowHandle InStyle, bool bInLoadStyle) {
...
TArray<FString> UCentralImage::GetCentralManagedPropertyNames() const {
	TArray<FString> Names;
	// As long as property names are equal on style and styled object, we can simply iterate.
	for (TFieldIterator<FProperty> It(FS_CentralImageStyle::StaticStruct()); It; ++It) {
		if (It != nullptr) {
...
bool UCentralImage::LoadCentralStyle() {
	const FS_CentralImageStyle* DTPtr = CentralStyle.GetRow<FS_CentralImageStyle>(TEXT("Loading Central Style"));
	if (DTPtr == nullptr) {
...
void UCentralImage::ActOnCentralStyleDataTableChanged() {
	LoadCentralStyle();
}
