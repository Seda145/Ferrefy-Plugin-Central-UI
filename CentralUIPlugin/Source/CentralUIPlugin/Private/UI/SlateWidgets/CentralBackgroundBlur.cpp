...
UCentralBackgroundBlur::UCentralBackgroundBlur(const FObjectInitializer& InObjectInitializer)
	: Super(InObjectInitializer) {

	ConstructorHelpers::FObjectFinderOptional<UDataTable> Obj(TEXT("/CentralUIPlugin/Data/DataTables/UI/Slate/BackgroundBlur/DT_Slate_BackgroundBlur.DT_Slate_BackgroundBlur"));
	if (Obj.Succeeded()) {
...
void UCentralBackgroundBlur::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent) {
...
void UCentralBackgroundBlur::OnWidgetRebuilt() {
	...
	*/
	TrackedCentralStyleDataTable = const_cast<UDataTable*>(GetCentralStyle().DataTable.Get());
	if (IsValid(TrackedCentralStyleDataTable)) {
...
FDataTableRowHandle UCentralBackgroundBlur::GetCentralStyle() const {
	return CentralStyle;
}

void UCentralBackgroundBlur::SetCentralStyle(FDataTableRowHandle InStyle, bool bInLoadStyle) {
...
TArray<FString> UCentralBackgroundBlur::GetCentralManagedPropertyNames() const {
	TArray<FString> Names;
	// As long as property names are equal on style and styled object, we can simply iterate.
	for (TFieldIterator<FProperty> It(FS_CentralBackgroundBlurStyle::StaticStruct()); It; ++It) {
		if (It != nullptr) {
...
bool UCentralBackgroundBlur::LoadCentralStyle() {
	const FS_CentralBackgroundBlurStyle* DTPtr = CentralStyle.GetRow<FS_CentralBackgroundBlurStyle>(TEXT("Loading Central Style"));
	if (DTPtr == nullptr) {
	...
	SetApplyAlphaToBlur(DTPtr->bApplyAlphaToBlur);
	SetBlurStrength(DTPtr->BlurStrength);
	SetLowQualityFallbackBrush(DTPtr->LowQualityFallbackBrush);
...
void UCentralBackgroundBlur::ActOnCentralStyleDataTableChanged() {
	LoadCentralStyle();
}
