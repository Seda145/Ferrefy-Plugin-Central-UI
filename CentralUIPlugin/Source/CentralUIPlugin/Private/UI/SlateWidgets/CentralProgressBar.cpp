...
UCentralProgressBar::UCentralProgressBar(const FObjectInitializer& InObjectInitializer)
	: Super(InObjectInitializer) {

	ConstructorHelpers::FObjectFinderOptional<UDataTable> Obj(TEXT("/CentralUIPlugin/Data/DataTables/UI/Slate/ProgressBar/DT_Slate_ProgressBar.DT_Slate_ProgressBar"));
	if (Obj.Succeeded()) {
...
void UCentralProgressBar::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent) {
...
void UCentralProgressBar::OnWidgetRebuilt() {
...
FDataTableRowHandle UCentralProgressBar::GetCentralStyle() const {
	return CentralStyle;
}

void UCentralProgressBar::SetCentralStyle(FDataTableRowHandle InStyle, bool bInLoadStyle) {
...
TArray<FString> UCentralProgressBar::GetCentralManagedPropertyNames() const {
	TArray<FString> Names;
	// As long as property names are equal on style and styled object, we can simply iterate.
	for (TFieldIterator<FProperty> It(FS_CentralProgressBarStyle::StaticStruct()); It; ++It) {
		if (It != nullptr) {
...
bool UCentralProgressBar::LoadCentralStyle() {
	const FS_CentralProgressBarStyle* DTPtr = CentralStyle.GetRow<FS_CentralProgressBarStyle>(TEXT("Loading Central Style"));
	if (DTPtr == nullptr) {
	...
	SetFillColorAndOpacity(DTPtr->FillColorAndOpacity);
	SetWidgetStyle(DTPtr->WidgetStyle);
	SetBarFillType(DTPtr->BarFillType);
	SetBarFillStyle(DTPtr->BarFillStyle);
	SetIsMarquee(DTPtr->bIsMarquee);
	SetBorderPadding(DTPtr->BorderPadding);
...
void UCentralProgressBar::ActOnCentralStyleDataTableChanged() {
	LoadCentralStyle();
}
