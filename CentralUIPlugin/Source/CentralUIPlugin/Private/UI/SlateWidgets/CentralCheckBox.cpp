...
UCentralCheckBox::UCentralCheckBox(const FObjectInitializer& InObjectInitializer)
	: Super(InObjectInitializer) {

	ConstructorHelpers::FObjectFinderOptional<UDataTable> Obj(TEXT("/CentralUIPlugin/Data/DataTables/UI/Slate/CheckBox/DT_Slate_CheckBox.DT_Slate_CheckBox"));
	if (Obj.Succeeded()) {
...
void UCentralCheckBox::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent) {
...
void UCentralCheckBox::OnWidgetRebuilt() {
...
FDataTableRowHandle UCentralCheckBox::GetCentralStyle() const {
	return CentralStyle;
}

void UCentralCheckBox::SetCentralStyle(FDataTableRowHandle InStyle, bool bInLoadStyle) {
...
TArray<FString> UCentralCheckBox::GetCentralManagedPropertyNames() const {
	TArray<FString> Names;
	// As long as property names are equal on style and styled object, we can simply iterate.
	for (TFieldIterator<FProperty> It(FS_CentralCheckBoxStyle::StaticStruct()); It; ++It) {
		if (It != nullptr) {
...
bool UCentralCheckBox::LoadCentralStyle() {
	const FS_CentralCheckBoxStyle* DTPtr = CentralStyle.GetRow<FS_CentralCheckBoxStyle>(TEXT("Loading Central Style"));
	if (DTPtr == nullptr) {
...
void UCentralCheckBox::ActOnCentralStyleDataTableChanged() {
	LoadCentralStyle();
}
