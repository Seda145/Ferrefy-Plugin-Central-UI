...
UCentralSpacer::UCentralSpacer(const FObjectInitializer& InObjectInitializer)
	: Super(InObjectInitializer) {

	ConstructorHelpers::FObjectFinderOptional<UDataTable> Obj(TEXT("/CentralUIPlugin/Data/DataTables/UI/Slate/Spacer/DT_Slate_Spacer.DT_Slate_Spacer"));
	if (Obj.Succeeded()) {
...
void UCentralSpacer::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent) {
...
void UCentralSpacer::OnWidgetRebuilt() {
...
FDataTableRowHandle UCentralSpacer::GetCentralStyle() const {
	return CentralStyle;
}

void UCentralSpacer::SetCentralStyle(FDataTableRowHandle InStyle, bool bInLoadStyle) {
...
TArray<FString> UCentralSpacer::GetCentralManagedPropertyNames() const {
	TArray<FString> Names;
	// As long as property names are equal on style and styled object, we can simply iterate.
	for (TFieldIterator<FProperty> It(FS_CentralSpacerStyle::StaticStruct()); It; ++It) {
		if (It != nullptr) {
...
bool UCentralSpacer::LoadCentralStyle() {
	const FS_CentralSpacerStyle* DTPtr = CentralStyle.GetRow<FS_CentralSpacerStyle>(TEXT("Loading Central Style"));
	if (DTPtr == nullptr) {
...
void UCentralSpacer::ActOnCentralStyleDataTableChanged() {
	LoadCentralStyle();
}
