...
UCentralSlider::UCentralSlider(const FObjectInitializer& InObjectInitializer)
	: Super(InObjectInitializer) {

	ConstructorHelpers::FObjectFinderOptional<UDataTable> Obj(TEXT("/CentralUIPlugin/Data/DataTables/UI/Slate/Slider/DT_Slate_Slider.DT_Slate_Slider"));
	if (Obj.Succeeded()) {
...
void UCentralSlider::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent) {
...
void UCentralSlider::OnWidgetRebuilt() {
...
FDataTableRowHandle UCentralSlider::GetCentralStyle() const {
	return CentralStyle;
}

void UCentralSlider::SetCentralStyle(FDataTableRowHandle InStyle, bool bInLoadStyle) {
...
TArray<FString> UCentralSlider::GetCentralManagedPropertyNames() const {
	TArray<FString> Names;
	// As long as property names are equal on style and styled object, we can simply iterate.
	for (TFieldIterator<FProperty> It(FS_CentralSliderStyle::StaticStruct()); It; ++It) {
		if (It != nullptr) {
...
bool UCentralSlider::LoadCentralStyle() {
	const FS_CentralSliderStyle* DTPtr = CentralStyle.GetRow<FS_CentralSliderStyle>(TEXT("Loading Central Style"));
	if (DTPtr == nullptr) {
...
void UCentralSlider::ActOnCentralStyleDataTableChanged() {
	LoadCentralStyle();
}
